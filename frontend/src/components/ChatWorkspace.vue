...
import { apiListHistory, type HistoryEntry, type PlanningResult, type PlanRequest } from '../api/agent'
...
const suggestions = [
  { label: '慢慢走', text: '规划一条南京老城南三小时的松弛漫步路线，想看老建筑，也想找一家安静咖啡馆' },
  { label: '亲子周末', text: '一家三口带6岁孩子，从新街口出发，安排一条轻松、有休息点的亲子路线' },
  { label: '文艺街巷', text: '想走一条有独立书店、展览和梧桐街道的文艺路线，避开太商业化的地方' },
]
...
function defaultPreferenceHint(): string {
  ...
  try {
    ...
    const value = JSON.parse(raw) as {
      pace?: string; transport?: string; familyFriendly?: boolean
      restStops?: boolean; avoidCrowds?: boolean; indoorFirst?: boolean
    }
    const labels = [
...
async function submit() {
  ...
  const uiContext = [
    selectedSkill.value
      ? `请在本轮使用用户配置的 Skill「${selectedSkill.value.name}」。\nSkill 说明：${selectedSkill.value.description}\n执行指令：${selectedSkill.value.instruction}`
      : '',
    attachedFiles.value.length ? `本轮附带文件：${attachedFiles.value.map(file => file.name).join('、')}。` : '',
    defaultPreferenceHint()
  ].filter(Boolean).join('\n')
  const requestTask = uiContext
    ? `${task}\n<citywalk_ui_context>\n${uiContext}\n</citywalk_ui_context>`
    : task
  const request: PlanRequest = {
    task: requestTask, preferredModel: 'flash', userId: getMemoryUserId(), threadId: getMemoryThreadId()
  }
...
function chooseSkill(skill: UserSkill) {
  selectedSkillId.value = selectedSkillId.value === skill.id ? '' : skill.id
...
async function loadHistory() {
  ...
  try { history.value = (await apiListHistory(userId, 30, 0)).entries } catch { history.value = [] }
...
<template>
  <main class="chat-workspace">
    <aside class="conversation-column">
      <div class="history-heading"><div><small>漫游记录</small><h2>最近对话</h2></div><button title="刷新对话" aria-label="刷新对话" @click="loadHistory"><PhArrowClockwise :size="17" /></button></div>
      <button class="new-chat" @click="newConversation"><PhPlus :size="18" weight="bold" /> 开始新漫游</button>
      <nav class="history-list">
        <p v-if="historyLoading && !history.length" class="history-state">正在翻阅旧手账…</p>
        <p v-else-if="!history.length" class="history-state">对话会像旅行便签一样留在这里。</p>
        <button v-for="entry in history" :key="entry.id" :class="{ active: activeHistoryId === entry.id }" @click="openHistory(entry)">
      ...
      <div class="history-foot"><span>对话记忆已开启</span><small>Agent 会延续当前对话中的约束</small></div>
    </aside>
    ...
    <section class="conversation-main paper-canvas">
      ...
      <div ref="streamEl" class="message-stream">
        <div v-if="!turns.length && agent.status.value === 'idle'" class="chat-welcome">
          ...
          <div class="suggestion-notes">
            <button v-for="(item, index) in suggestions" :key="item.label" @click="taskText = item.text"><span>{{ String(index + 1).padStart(2, '0') }} / {{ item.label }}</span><p>{{ item.text }}</p></button>
          </div>
        </div>
      ...
      <div class="composer-dock">
        <div class="torn-edge" />
        <div class="composer-box" :class="{ expanded: composerFocused || Boolean(taskText) || Boolean(selectedSkillId) || attachedFiles.length }" @focusin="composerFocused = true">
          ...
          <div class="composer-tools">
            <div class="skill-picker">
              <button class="tool-button" :class="{ active: selectedSkillId }" @click="skillMenuOpen = !skillMenuOpen"><PhSparkle :size="15" /> {{ selectedSkill?.name || '添加 Skill' }}</button>
              <div v-if="skillMenuOpen" class="skill-menu">
                <button v-for="skill in availableSkills" :key="skill.id" :class="{ active: selectedSkillId === skill.id }" @click="chooseSkill(skill)"><strong>{{ skill.name }}</strong><small>{{ skill.description }}</small></button>
